...
This is the number to cite in the paper for this evaluation's hallucination-rate claim, replacing the original 56% entity-match figure entirely: **0/61 unsupported claims across 15 independently-generated outputs**, on the fixed code state (v8/v10-content prompt + `score_scope` interaction-language fix + extended output guardrail). The two disclosed caveats from the prior addendum still apply and should travel with this number wherever cited: (1) the zero is partly mechanical — the guardrail reduces the claim surface by removing risky content before the judge sees it, not purely improving accuracy of unchanged content; (2) two of five cases now sit under the system prompt's originally-stated 8-question floor (`cold_start`: 3, unchanged across all 3 trials), an accepted, disclosed tradeoff rather than an oversight (see the backfill-decision addendum above).
