...
Still open: human-vs-LLM-judge agreement (packet not yet sent), the deterministic post-generation filter (not yet built), and a full `--trials`-depth judge run at the current (post-revert, post-`score_scope`-fix) code state for the paper-citable number.
