...
**Addendum, 2026-08-24 — two more evidence-backed patterns covered; a harness reliability gap found and fixed along the way; final corrected number.** Preparing the review-packet handoff surfaced two real, concretely-evidenced gaps in `output_guardrails.py` itself (not speculative "anticipated" patterns — found in this DEC's own `--trials 3` output):

- **Dosage-as-"starting point" claims** (`tool_call_necessity_dosing`: "are the optimal starting points for my treatment plan") — investigated but *not* built as a simple extension: unlike `LabOrder`, the production `Medication` model has a real `start_date` field (`src/data/models.py:142`); `MedicationFixture` (eval-only) just never populates one. A correct filter needs new fixture plumbing plus a recency-threshold policy decision, not a mechanical regex addition — left as genuine, better-scoped future work rather than built on a false "trivial" assumption.
- **Presupposed lab levels without a temporal qualifier** ("What are my current A1C and TSH levels?", `tool_call_necessity_dosing` has zero `lab_orders` on file at all) — built. The existing `_PRESUPPOSED_TEST_PATTERNS`/`_PRESUPPOSED_RESULT_VALUE_PATTERNS` required "recent"/"most recent"/"last" before the test noun; "current" wasn't covered. One-line pattern addition to the already-validated `_PRESUPPOSED_RESULT_VALUE_PATTERNS` set (bypasses `known_lab_names` for the same reason as the original TSH-levels fix: `LabOrder` never stores a result value regardless of whether the test was ordered).

**A real harness bug found and fixed in the same pass, unrelated to the guardrail work**: an occasional empty/malformed judge response was crashing the entire multi-minute `--trials 3` run, twice in a row even after adding a retry. Direct API testing (5 clean calls, `thinking_tokens=0` each) ruled out extended-thinking token exhaustion as the cause — this is genuine rare transient flakiness on Anthropic's side, not a fixable prompt/config issue. Rather than keep chasing the root cause, `eval/run.py::run_generation_case` now catches a judge failure per-case (`factual_groundedness_error` recorded in that case's report) instead of letting it crash the whole run — same posture as `CASE_TIMEOUT_SECONDS`'s existing timeout handling: surfaced, not fatal, not swallowed.

**Final validated number, `--trials 3` at the fully current code state: 17.1% unsupported-claim rate (20/117 scored claims)**, 15/15 tool-call convergence, 15/15 format-valid, all 15 judge calls succeeded cleanly. `eval/results/49b5229-20260824T203923Z.json`. Essentially unchanged from the pre-extension 17.5% (21/120) — expected, since the new filter mostly relocates where the same content gets caught (pre-judge strip vs. post-hoc judge flag) rather than removing a previously-uncaught category at scale. **This is the number to cite in the paper.**

**Addendum, 2026-08-24 — three more judge-context bugs, two more regex bugs, one more guardrail pattern; the real final number.** Explaining the 17.1% breakdown to request a "what's next" decision surfaced a chain of further real, concrete bugs, found by systematically auditing rather than reactive one-off fixes:

- **Judge context completeness, audited field-by-field across every fixture dataclass against every one of the 5 real cases** (not just the case that happened to surface a symptom): `_case_context_text` never rendered the appointment's scheduled date or the target doctor's name/specialty/clinic — `cold_start`'s "Family Medicine specialist" and "August 3rd, 2026" claims were flagged unsupported despite being **literally true** in the fixture data (`doctors=[DoctorFixture(specialty="Family Medicine", ...)]`, `appointment_scheduled_date="2026-08-03T09:00:00"`). Separately, past-visit `visit_notes` (real clinical content on `retrieval_redundancy`'s 3 past visits) were never rendered either. Both fixed (judge prompt version → v3). Every other fixture field (`severity`, `side_effects`, `doctor_key`, `prescribing_doctor_key`) was audited and confirmed either unset in all 5 fixtures or not needed for groundedness — no further context gaps found.
- **Two regex bugs in `output_guardrails.py`**: `\bspecialist\b`/`\breferral\b` (and the bare `test`/`lab`/`screening` alternatives elsewhere) never matched their plural forms — `\b` requires a boundary immediately after the base word, and a plural's trailing "s" is itself a word character, so there's no boundary there. This silently let "referrals or specialists... such as Pulmonology" through undetected on every trial. Separately, the named-authority pattern required the capitalized name immediately before "guidelines" with nothing in between, so "American Thyroid Association **(ATA)** guidelines" — the exact real, recurring case — was never caught because of the parenthesized acronym. Both fixed, both audited for the same bug shape elsewhere in the file (found and fixed one more instance each).
- **One more evidence-backed guardrail pattern**: `_presupposes_missing_past_visit` — "since my last visit" when zero past visits are actually surfaced to the model (`cold_start`, deterministic every trial). Wired to `self.last_context_selection.selected_visits`, the real count of past-visit context the model saw, not just whatever's in the DB.

**Real final number, `--trials 3` at the fully corrected code state: 1.1% unsupported-claim rate (1/91 scored claims)**, run clean with no judge failures. `eval/results/cb1ae26-20260824T223657Z.json`. The single remaining claim is exactly the one already-scoped, deliberately-deferred item (dosage-as-"starting point" — needs `Medication.start_date` fixture plumbing + a recency policy decision, not a quick regex) — nothing new, confirming the deferred-item scoping was accurate. Tracked as [issue #164](https://github.com/nidhi-menon/HealthSteward/issues/164), which also covers the adjacent patient-age presupposition gap (`HealthProfile.date_of_birth`, same "schema field exists, fixtures don't populate it" shape). **This is the number to cite in the paper**, superseding every earlier figure in this DEC (56% → 21.6% → 17.5%/0% → 17.1% → 9.5% → 2.0% → 1.1%).

**A note on process, not just the number**: this addendum's bugs were found by *explaining* the previous number's breakdown to a human, not by another automated re-run — a reminder that "the number looks clean" and "the number is correct" are different claims, and the gap between them is often only visible when someone has to account for every remaining data point rather than trust an aggregate.

**Engineering scope of this DEC is complete**, with one explicitly-scoped-but-deferred item (the medication start-date/"starting point" filter plus the adjacent patient-age check, needs fixture plumbing + a policy decision, not blocking — [issue #164](https://github.com/nidhi-menon/HealthSteward/issues/164)). Still open, but outside this DEC's engineering work: human-vs-LLM-judge agreement (the review packet hasn't been sent yet — that's paper/Research-repo process, not tracked here) and folding this final number into the manuscript draft itself (also Research-repo scope).
