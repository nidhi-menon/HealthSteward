...
Still open: human-vs-LLM-judge agreement (packet not yet sent), and a full `--trials`-depth judge run at the current (fully fixed: v8 prompt, `score_scope` fix, output guardrail all active) code state for the final paper-citable number — the numbers in this addendum are noise-isolated re-judges of one generation pass each, not yet a multi-trial run across all 5 cases independently regenerated.
