...
Still open: human-vs-LLM-judge agreement (packet not yet sent), and a full `--trials`-depth judge run at the current (final: v8/v10-content prompt, `score_scope` fix, extended output guardrail all active) code state for the final paper-citable number — the numbers in this and the prior two addenda are single-pass or noise-isolated re-judges, not yet a multi-trial run across all 5 cases independently regenerated.
