...
**Real final number, `--trials 3` at the fully corrected code state: 1.1% unsupported-claim rate (1/91 scored claims)**, run clean with no judge failures. `eval/results/cb1ae26-20260824T223657Z.json`. The single remaining claim is exactly the one already-scoped, deliberately-deferred item (dosage-as-"starting point" — needs `Medication.start_date` fixture plumbing + a recency policy decision, not a quick regex) — nothing new, confirming the deferred-item scoping was accurate. **This is the number to cite in the paper**, superseding every earlier figure in this DEC (56% → 21.6% → 17.5%/0% → 17.1% → 9.5% → 2.0% → 1.1%).
...
**Engineering scope of this DEC is complete**, with one explicitly-scoped-but-deferred item (the medication start-date/"starting point" filter, needs fixture plumbing + a policy decision, not blocking). Still open, but outside this DEC's engineering work: human-vs-LLM-judge agreement (the review packet hasn't been sent yet — that's paper/Research-repo process, not tracked here) and folding this final number into the manuscript draft itself (also Research-repo scope).
