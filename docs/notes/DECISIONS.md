...
**Known limitations, not solved by this design (carried forward from the PACT review discussion):**
...
- **No automatic loop-closure.** Even if a contact acts on an escalation, HealthSteward has no account/API for them to write back — `status` stays `pending` until the patient (or someone who tells the patient) updates it in-app. Escalation shortens the gap between "task overdue" and "someone besides the patient knows," it does not close the confirmation loop itself.
...
**Best-available mitigations for those two limitations, to build into implementation (not deferred):**
- **Double opt-in before automated sends begin.** The first message to any new `CareContact` is a one-time, low-content confirmation ("HealthSteward will send you updates about [patient]'s care — reply STOP to opt out") rather than a live escalation. Recurring automated sends to that contact only start after that email doesn't bounce/opt-out — this is the standard email-consent pattern and directly addresses the identity/consent risk without requiring any authentication infrastructure.
- **Message content minimization.** Escalation emails carry task category and how-overdue, not clinical detail (e.g. "a referral is overdue" not "referral to Dr. X for [condition]"). Reduces PHI exposure to metadata regardless of who the recipient turns out to be.
- **No write-back is attempted.** Rather than half-build a fragile mechanism (e.g. a reply-parsing or one-time-link "mark as done" flow) that would require the local app to be network-reachable — itself a much larger architectural shift away from local-first — the escalation email's own copy should explicitly ask the recipient to tell the patient directly, and the in-app confirmation step remains the sole source of truth for `status`. Making this limitation explicit in the UX (rather than implying automation that isn't there) is the honest ceiling given the local-first constraint.
...
**Status:** Proposed. Not implemented. No issue filed yet — filing one (against/superseding #25) is the next step if this direction is approved.
