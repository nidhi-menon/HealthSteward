...
class VisitPrepAgent(BaseAgent):
    ...
    SYSTEM_PROMPT_TEMPLATE_VERSION = "v7-2026-08-24"
...
- Do NOT recommend or ask about scheduling a follow-up or referral with a specialist unrelated to {specialty}, even for a condition that specialist genuinely treats — that follow-up belongs to a future appointment with them, not this one. A cross-specialty *interaction* question is still in scope (e.g. asking whether a medication {specialty} manages interacts with one another specialist prescribed); recommending or scheduling that other specialist's own visit is not.
...
- Do NOT presuppose that a specific test, lab, or referral exists or was already performed unless it is explicitly listed below. A question like "What were the results of my most recent [X] test?" asserts that test happened — only phrase a question that way when the test/lab is actually listed in the patient data. If you want to raise something that might be relevant but wasn't provided, ask an open question instead (e.g. "Are there any tests or labs I should be getting related to X?"), not one that assumes it already exists.
...
Before finalizing your response, count your questions. You must have between 8 and 15 total across all categories combined — if you're short, add more within your existing (non-empty) categories rather than reintroducing an empty one; if you're over 15, cut down to the 15 most clinically useful ones, not just the first 15 you generated. Be specific — reference actual condition names, medication names, and lab test names from the patient data provided."""
    ...
    SYSTEM_PROMPT_GENERIC_VERSION = "v7-2026-08-24"
